...
impl UserData {
    ...
    pub fn on_deposit(&mut self, resource_address: ResourceAddress, sd_deposit_increase: Decimal) {
        let mut sd_deposit = self.get_deposit(resource_address);
        sd_deposit += sd_deposit_increase;
        self.update_deposit(resource_address, sd_deposit);
    }

    pub fn on_withdraw(&mut self, resource_address: ResourceAddress, sd_deposit_decrease: Decimal) {
        let mut sd_deposit = self.get_deposit(resource_address);
        sd_deposit -= sd_deposit_decrease;
        if sd_deposit < Decimal::ZERO {
            panic!("Amount is greater than deposit balance");
        }
        self.update_deposit(resource_address, sd_deposit);
    }

    pub fn on_borrow(&mut self, resource_address: ResourceAddress, sb_borrow_increase: Decimal) {
        let mut sb_borrow = self.get_borrow(resource_address);
        sb_borrow += sb_borrow_increase;
        self.update_borrow(resource_address, sb_borrow);
    }

    pub fn on_repay(&mut self, resource_address: ResourceAddress, sb_borrow_decrease: Decimal) {
        let mut sb_borrow = self.get_borrow(resource_address);
        sb_borrow -= sb_borrow_decrease;
        if sb_borrow < Decimal::ZERO {
    ...
    pub fn on_liquidate_repay(
        ...
        let sb_borrow = self.get_borrow(resource_address);
        // Increase borrow balance by interests accrued
        let mut new_sb_borrow = sb_borrow * sb_price;
        let mut interest = new_sb_borrow - sb_borrow;
        interest *= cost_of_asset_in_terms_of_xrd;
        new_sb_borrow *= cost_of_asset_in_terms_of_xrd;
        // Repay the loan
        if new_sb_borrow < amount {
            panic!("Amount is greater than borrow balance");
        } else {
            new_sb_borrow -= amount;
            new_sb_borrow /= cost_of_asset_in_terms_of_xrd;
            self.update_borrow(resource_address.clone(), new_sb_borrow);
            interest
        }
    }
    ...
    pub fn calculate_total_collateral_and_loan(
        ...
        for (asset_address, parameters) in pool_parameters {
            ...
            let sd_deposit_balance = self.get_deposit(asset_address.clone());
            if sd_deposit_balance != Decimal::ZERO {
                let sd_price = parameters.deposit_balance / parameters.sd_deposit_balance;
                let ltv_ratio = &parameters.ltv_ratio;

                let asset_value_in_xrd =
                    sd_deposit_balance * sd_price * *cost_of_asset_in_terms_of_xrd;
                let asset_collateral = asset_value_in_xrd * *ltv_ratio;
                user_collateral_sum += asset_collateral;
            }
            let sb_borrow_balance = self.get_borrow(asset_address.clone());
            if sb_borrow_balance != Decimal::ZERO {
                let sb_price = parameters.borrow_balance / parameters.sb_borrow_balance;
                let asset_loan = sb_borrow_balance * sb_price * *cost_of_asset_in_terms_of_xrd;
